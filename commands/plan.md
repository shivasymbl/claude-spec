...
IF ARG_TYPE == "new_seed":
  ...
  → PROCEED to Step 0.5: Triage Gate
```

<triage_gate>

### Step 0.5: Triage Gate

Before creating any workspace, determine the work type. This controls elicitation depth and artifact requirements.
...
Use AskUserQuestion with:
  ...
  options:
    - label: "Fix / Bug"
      description: "Observable broken behavior — skip discovery, go direct to evidence and implement"
    - label: "Feature"
      description: "New capability, enhancement, or user-visible change"
    - label: "System / Architecture"
      description: "New architecture, multi-service change, or foundational work"
```

Store result as WORK_TYPE (fix | feature | system).
Set SHAPIRO_LEVEL: fix=2, feature=4, system=5.
Store in-memory for now — write to README.md frontmatter during workspace initialization (after Step 1).

THEN → PROCEED to Step 1: Check Current Branch
...
1. **Specification documents** in `docs/spec/active/{project}/`:
   ...
   - REQUIREMENTS.md (PRD)
   - SCENARIOS.md (holdout validation — NOT read by implementer during build)
...
Question 5 (features and systems only — skip for fixes):
  header: "Scope"
  question: "What should this explicitly NOT include? Explicit exclusions prevent scope creep."
  multiSelect: true
  options:
    - label: "Admin / config UI"
      description: "Back-office tooling or configuration screens"
    - label: "Data migration"
      description: "Backfilling or transforming historical records"
    - label: "Third-party integrations beyond core"
      description: "Additional connectors or platforms not in immediate scope"
    - label: "Performance optimization pass"
      description: "First version ships correct; performance is a follow-on"
```
...
status: draft
shapiro_level: ${SHAPIRO_LEVEL}  # 2=fix | 4=feature | 5=system — set during Step 0.5 triage
...
status: draft
shapiro_level: ${SHAPIRO_LEVEL}  # 2=fix | 4=feature | 5=system
---
...
### SCENARIOS.md — Holdout Validation (Required for features and systems)

After generating REQUIREMENTS.md, immediately generate SCENARIOS.md using the template at `./assets/scenarios-template.md`.

**Critical rule:** SCENARIOS.md is the evaluation set. The implementer (claude running /implement) must NEVER read this file during build phases. Only the convergence gate reads it post-implementation.

Generate:
- 1+ happy path scenario per feature (normal user, goal succeeds)
- 1+ edge case scenario per feature (boundary conditions, unusual input)
- 1 adversarial variant per critical feature (attempt to break or misuse)
- For fixes: regression scenarios proving the fixed behavior still works

Satisfaction target: 90%+ Satisfied across all scenarios at convergence.
...
- [ ] "What This Is NOT" documented for each feature (Scope Exclusions)
- [ ] SCENARIOS.md exists with 2+ scenarios per feature (features/systems)
- [ ] SCENARIOS.md contains NO cross-references to REQUIREMENTS.md acceptance criteria
      </quality_gates>
