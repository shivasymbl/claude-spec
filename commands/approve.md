...
SCENARIOS_EXISTS=$([ -f "${PROJECT_DIR}/SCENARIOS.md" ] && echo "true" || echo "false")
...
echo "SCENARIOS.md: ${SCENARIOS_EXISTS}"
```
...
```
IF any required document is missing (README, REQUIREMENTS, ARCHITECTURE, IMPLEMENTATION_PLAN):
  -> Display error:
...
IF SCENARIOS_EXISTS == "false" AND shapiro_level >= 4 (feature or system):
  -> Display warning:
    "⚠️ SCENARIOS.md is missing. Holdout validation requires SCENARIOS.md for features and systems.
     Without it, the convergence gate in /implement will be skipped and satisfaction cannot be
     independently verified."
  -> Use AskUserQuestion:
       header: "Missing SCENARIOS.md"
       question: "SCENARIOS.md is absent. How do you want to proceed?"
       multiSelect: false
       options:
         - label: "Return to plan — generate SCENARIOS.md first"
           description: "Run /claude-spec:plan to add holdout scenarios before approving (recommended)"
         - label: "Approve without holdout validation"
           description: "Downgrade to shapiro_level 2: convergence gate will be skipped in /implement"
  -> IF user chose "Approve without holdout":
       Set shapiro_level: 2 in README.md frontmatter before writing approval
     ELSE:
       STOP — return control to user to run /plan
