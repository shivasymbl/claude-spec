...
**SCENARIOS.md exclusion rule:**
Never read, reference, or load SCENARIOS.md during implementation phases.
This file is the holdout evaluation set — implementer access destroys its validity.
The convergence gate reads it AFTER all implementation is complete.
</implementation_gate>
...
| SCENARIOS.md           | Convergence gate run   | Fill scoring summary table     |
...
### Convergence Gate (features and systems with Shapiro Level >= 4)

**Pre-step — load shapiro_level:**

```bash
SHAPIRO_LEVEL=$(grep "^shapiro_level:" "${PROJECT_DIR}/README.md" 2>/dev/null | awk '{print $2}' | tr -d '"' || echo "4")
```

Default to `4` (feature) if missing.

**Skip if:** No SCENARIOS.md exists OR shapiro_level < 4 (i.e., "fix", "2", or "3").

**CRITICAL — SCENARIOS.md exclusion rule:**
Do NOT read SCENARIOS.md during implementation phases 0-N. This file is reserved exclusively for the convergence gate. Reading it during build destroys the holdout validation property.

**After all implementation tasks complete:**

1. Check: `[ -f "${PROJECT_DIR}/SCENARIOS.md" ]`
   - If absent: skip convergence gate, proceed to documentation gate

2. If present: Execute each scenario from SCENARIOS.md against the implementation
   Score as: Satisfied / Partially Satisfied / Not Satisfied

3. Calculate: satisfied_count / total >= 0.90 AND zero "Not Satisfied" on critical paths

4. If PASSES (>=90%): proceed to documentation gate
   If FAILS (<90%): create new fix tasks in PROGRESS.md, re-implement, re-run (max 3 iterations)

5. After 3 failed iterations: escalate to user — spec needs revision, not more code
...
IF project_status == "completed":
  -> Run CONVERGENCE GATE first (see "Convergence Gate" section above)
     IF convergence gate fails after 3 iterations: STOP — do NOT proceed to cleanup
  -> Only after convergence passes (or was skipped for level < 4 / absent SCENARIOS.md):
  -> Skip user confirmation
