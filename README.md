...
- **Approval Workflow** - Governance controls
  - SCENARIOS.md validation — holdout scenarios required for feature/system specs
  - PreToolUse hook (`hooks/check-approved-spec.sh`) — optional governance gate (disabled by default)
  - Audit trail with approver identity, timestamps, and shapiro_level
...
```
/plugin marketplace add asymblinc/claude-spec
...
```bash
git clone https://github.com/asymblinc/claude-spec.git
cd claude-spec
...
- `docs/spec/active/YYYY-MM-DD-project-slug/`
  ...
  - REQUIREMENTS.md    (with Discovery Summary and Scope Exclusions)
  - SCENARIOS.md       (holdout validation — hidden from implementer during build)
  - ARCHITECTURE.md
...
Before elicitation, `/plan` now classifies work type via an AskUserQuestion:

| Type | Shapiro Level | Protocol |
|---|---|---|
| Fix / Bug | 2 | Evidence-first → lightweight spec |
| Feature | 4 | Full elicitation (5 questions incl. scope exclusions) + SCENARIOS.md |
| System / Architecture | 5 | Full elicitation + system context reference + SCENARIOS.md |
...
/claude-spec:report-issue --type feat --repo asymblinc/claude-spec
```
...
```
/claude-spec:plan "idea"     Step 0.5: Triage (Fix/Feature/System)
          |                   Full elicitation + SCENARIOS.md (feature/system)
          |
/claude-spec:approve slug    Validates: REQUIREMENTS + SCENARIOS + ARCHITECTURE + IMPLEMENTATION_PLAN
          |
/claude-spec:implement slug  Tracks implementation
          |                   NEVER reads SCENARIOS.md during build (holdout property)
...
3. **SCENARIOS.md holdout** — implementer never reads this file; convergence gate validates post-build
4. **PreToolUse hook (`hooks/check-approved-spec.sh`)** — optional blocking of Write/Edit without approved spec (disabled by default, opt-in)
...
The PreToolUse hook is an optional governance gate (disabled by default) configured in `.claude-plugin/hooks.json`:
  ...
  "hooks": [
    {
      ...
      "description": "Optional governance gate: blocks Write/Edit without approved spec",
      "event": "PreToolUse",
      "matcher": "Write|Edit|str_replace_editor",
      "type": "command",
      "command": "hooks/check-approved-spec.sh",
      "blocking": true,
      "disabled": true
    }
  ]
...
assets/              # SaCP spec templates
+-- scenarios-template.md
+-- requirements-template.md
+-- implementation-template.md
references/          # SaCP methodology reference library
+-- interview-protocol.md
+-- evidence-protocol.md
+-- system-context-protocol.md
+-- null-contract.md
hooks/               # Optional governance hooks
+-- check-approved-spec.sh
```
